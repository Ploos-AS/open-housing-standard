...
mkdir -p src/ohs tests/editorial

cat > src/ohs/frontmatter.py <<'PY'
...
entry = 'ohs-normalize-frontmatter = "ohs.frontmatter:main"'
...
if entry not in text:
    ...
    print("[+] Added ohs-normalize-frontmatter to pyproject.toml")
else:
    print("[=] CLI already configured")
PY

cat > tests/editorial/test_frontmatter.py <<'PY'
...
echo
echo "[✓] Frontmatter normalizer installed."
echo "Preview:"
echo "  uv run ohs-normalize-frontmatter --check"
echo "Apply:"
echo "  uv run ohs-normalize-frontmatter --fix"
echo "Validate:"
